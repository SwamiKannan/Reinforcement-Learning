...
class ReplayBuffer():
    ...
    def sample(self):
        ...
        if len(self.buffer)<self.batch_size:
            ...
            states_=torch.from_numpy(np.vstack([e.state for e in sample_idx if e is not None])).float()
            actions_=torch.from_numpy(np.vstack([e.action for e in sample_idx if e is not None])).long()
            rewards_=torch.from_numpy(np.vstack([e.reward for e in sample_idx if e is not None])).float()
            next_states_=torch.from_numpy(np.vstack([e.next_state for e in sample_idx if e is not None])).float()
            dones_=torch.from_numpy(np.vstack([e.done for e in sample_idx if e is not None]).astype(np.uint8)).float().squeeze()
